// Adapted from https://github.com/LonelyWolf/stm8/
...
uint8_t nRF24_ReadWrite(uint8_t data) {
    while (!SPI_GetFlagStatus(SPI_FLAG_TXE)); // Wait while DR register is not empty
    SPI_SendData(data); // Send byte to SPI
    while (!SPI_GetFlagStatus(SPI_FLAG_RXNE)); // Wait to receive byte
...
uint8_t nRF24_TXPacket(uint8_t * pBuf, uint8_t TX_PAYLOAD) {
    ...
    /*while(PB_IDR & 0x2); // Wait for IRQ from nRF24L01*/
    /*while(status == nRF24_ReadReg(nRF24_REG_STATUS));*/
    /*for(i = 0; i < 10000; i++){*/
    /*}*/
    while(!(status & (nRF24_MASK_RX_DR | nRF24_MASK_TX_DS | nRF24_MASK_MAX_RT))){
        status = nRF24_ReadReg(nRF24_REG_STATUS); // Read status register
    }
    CE_L();
    status = nRF24_ReadReg(nRF24_REG_STATUS); // Read status register
